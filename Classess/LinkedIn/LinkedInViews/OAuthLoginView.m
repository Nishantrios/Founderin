...
//#define OAUTH_CONSUMER @"75h8qgr501kw22"
//#define OAUTH_ACCESSTOKEN @"SsNnCgqyp7rn7zIk"
...
- (void)requestTokenFromProvider
{
    ...
    /*OARequestParameter *nameParam = [[OARequestParameter alloc] initWithName:@"scope"
     value:@"r_fullprofile+rw_nus+r_network+r_contactinfo"];*/
    OARequestParameter *nameParam = [[OARequestParameter alloc] initWithName:@"scope"
                                                                       value:@"r_fullprofile+r_contactinfo+r_emailaddress+w_messages+r_contactinfo+r_network+rw_nus"];
    NSArray *params = [NSArray arrayWithObjects:nameParam, nil];
    [request setParameters:params];
    // OARequestParameter * scopeParameter=[OARequestParameter requestParameter:@"scope" value:@"r_fullprofile rw_nus r_network r_contactinfo"];
    OARequestParameter * scopeParameter=[OARequestParameter requestParameter:@"scope" value:@"r_fullprofile r_contactinfo r_network rw_nus"];
    
    [request setParameters:[NSArray arrayWithObject:scopeParameter]];
    //[appSharedData showCustomLoaderWithTitle:@"Please wait ..."];
...
- (void)requestTokenResult:(OAServiceTicket *)ticket didFinish:(NSData *)data
{
    ...
    NSLog(@"%s,RequestTokenFromProvider responseBody:%@",__FUNCTION__, responseBody);
    /*
     //TODO: Change code for request token
...
{
    //TODO : check- Set access token nil
...
- (void)accessTokenResult:(OAServiceTicket *)ticket didFinish:(NSData *)data
{
    NSString *responseBody = [[NSString alloc] initWithData:data
                                                   encoding:NSUTF8StringEncoding];
    NSLog(@"%s,AccessTokenFromProvider responseBody:%@",__FUNCTION__, responseBody);
...
- (void)accessTokenResult:(OAServiceTicket *)ticket didFail:(NSData *)data
{
    ...
    NSString *responseBody = [[[NSString alloc] initWithData:data encoding:NSASCIIStringEncoding] autorelease];
...
{
    /*
     apikey    = @"75vm02yj08bx27";
     secretkey = @"I7phOkuoVtGok0r1";
     */
    apikey    = API_KEY_LINKEDIN;
    secretkey = API_SECRETKEY_LINKEDIN;
